...
class Posts{
    ...
    public function submitPost($body, $user_to)
    {
        ...
        $check_empty=preg_replace('/\s+/',"",$body);//delete all space<?php
class Post {
	private $user_obj;
	private $conn;
	...
	public function loadPostsFriends($data, $limit) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$user_Logged_obj=new User($this->conn,$userLogedin);
...
?>
					<script>
					//js function to display comment frame
...
<?php				$comment_check= mysqli_query($this->conn, "SELECT * FROM comments WHERE post_id ='$id'");
					...
					$str .= "<div class='status_post' onClick='javascript:toggle$id()'>
					         <!--onclick() isnt working nd I dnt know y-->
